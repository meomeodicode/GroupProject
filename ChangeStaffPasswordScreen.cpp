...
void changePasswordCSV (string idInput, string newPassword)
{
    ifstream fin ("Src/FileCSV/AccountStaff.csv");
    ofstream fout ("Src/FileCSV/AccountStaff_temp.csv");
    string fileString = "";
    ...
    if (fin.is_open() && fout.is_open()) {
        ...
        if (remove("Src/FileCSV/AccountStaff.csv") != 0) {
            perror("Error deleting file");
        }

        // rename the temp file to the original file name
        if (rename("Src/FileCSV/AccountStaff_temp.csv", "Src/FileCSV/AccountStaff.csv") != 0) {
            perror("Error renaming file");
        }
    }
}

void ChangeStaffPasswordScreen (RenderWindow &app, string idInput, string passwordInput)
{
    string inputTmp = "";
    int check = 0;
            ...
            else if (e.type == Event::KeyPressed && e.key.code == Keyboard::Enter)
            {
                ...
                if (inputTextOldPassword.getString() != passwordInput) {
                    ...
                    if (inputTextNewPassword1.getString() != inputTextNewPassword2.getString() || inputTextNewPassword1.getString() == inputTextOldPassword.getString() || inputTextNewPassword2.getString() == inputTextOldPassword.getString()) {
                        ...
                        changePasswordCSV (idInput, inputTextNewPassword1.getString());
